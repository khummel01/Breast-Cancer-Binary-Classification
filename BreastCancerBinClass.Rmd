...
This analysis shows that the probability of a breast mass being malignant is most highly correlated with log(worst radius), worst concavity, mean smoothness, log(standard error of texture), log(mean compactness), and worst fractal dimension. The odds of a breast mass being malignant increases the most with each additional unit of mean smoothness where each additional unit of mean smoothness increases the odds of a breast mass being malignant by approximately 4.7 times. The least influential factor is log(SE texture) where each additional unit increase in this predictor only increases the odds of a breast mass being malignant by 1.02 times. With an AUC value of 0.9935208, our model can accurately predict whether or not a breast mass is malignant or benign 99.25% of the time given this data set. These results could potentially be used in the medical field to help predict whether a given breast mass is malignant or benign. 

Some follow-up questions could include:
-Is this dataset representative of all types of breast cancers?
-Is the scope of this dataset large enough to conclude on the predictative abilities of each predictor variable?
